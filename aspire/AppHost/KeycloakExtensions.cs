...
public static class KeycloakExtensions
{
    ...
    public static IResourceBuilder<KeycloakResource> RunWithHttpsDevCertificate(
        this IResourceBuilder<KeycloakResource> builder, int targetPort = 8443)
    {
        ...
        IEnumerable<KeycloakClientDetails> clientDetails)
    {
        builder
            ...
            .WithEnvironment("REALM_HSTS",
                builder.ApplicationBuilder.ExecutionContext.IsRunMode ? "" : "max-age=31536000; includeSubDomains");
        ...
        foreach (var client in clientDetails)
        {
            ...
            // 仅当 ClientSecret 不为 null 时生成环境变量
            if (client.ClientSecret != null)
            {
                builder.WithEnvironment($"CLIENT_{client.EnvironmentVariable}_SECRET", client.ClientSecret);
            }
            ...
            builder.WithEnvironment(context =>
            {
                ...
                context.EnvironmentVariables[$"CLIENT_{client.EnvironmentVariable}_URL"] =
                    context.ExecutionContext.IsPublishMode ? httpsEndpoint : httpsEndpoint.Url;
                context.EnvironmentVariables[$"CLIENT_{client.EnvironmentVariable}_URL_CONTAINERHOST"] = httpsEndpoint;

                // 新增：回调 URL 注入
                var redirectPath = "/authentication/callback"; // 你的前端回调路径
                context.EnvironmentVariables[$"CLIENT_{client.EnvironmentVariable}_REDIRECT_URL"] =
                    $"{httpsEndpoint.Url.TrimEnd('/')}{redirectPath}";

                // webOrigins 注入
                context.EnvironmentVariables[$"CLIENT_{client.EnvironmentVariable}_WEB_ORIGINS_URL"] =
                    httpsEndpoint.Url; // 如果需要多个 origin，可以用逗号或数组形式注入
            });
        }
